...
class NeuralNetwork():
    ...
    def draw(self, directory, epoch, parent, idx, fitness, weights_list=None):
        ...
        pyplot.gca().text(0.05, 0.99, f"Fitness: {fitness}", transform=pyplot.gca().transAxes, fontsize=10, verticalalignment='top', bbox=props)
...
class DrawNN():
    ...
    def save_draw( self, epoch, parent, idx, fitness):
        ...
        for l in self.neural_network:
            network.add_layer(l)
        network.save_draw(self.directory, epoch, parent, idx, fitness, self.weights_list)