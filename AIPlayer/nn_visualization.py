...
class Layer:
    ...
    def __line_between_two_neurons(
        self, neuron1, neuron2, weight=0.4, textoverlaphandler=None
    ):
        ...
        if abs_weight > 0.5:
            ...
            txt_x_pos = (
                neuron1.x
                - x_adjustment
                + index_step * (neuron2.x - neuron1.x + 2 * x_adjustment) / num_segments
            )
            txt_y_pos = (
                neuron1.y
                - y_adjustment
                + index_step * (neuron2.y - neuron1.y + 2 * y_adjustment) / num_segments
            )
            while (
                not textoverlaphandler.getspace(
                    [txt_x_pos - 0.5, txt_y_pos - 0.5, txt_x_pos + 0.5, txt_y_pos + 0.5]
                )
                ...
                txt_x_pos = (
                    ...
                    - x_adjustment
                    + index_step
                    * (neuron2.x - neuron1.x + 2 * x_adjustment)
                    / num_segments
                )
                txt_y_pos = (
                    neuron1.y
                    - y_adjustment
                    + index_step
                    * (neuron2.y - neuron1.y + 2 * y_adjustment)
                    / num_segments
                )

            # print("Label positions: ", "{:.2f}".format(txt_x_pos), "{:.2f}".format(txt_y_pos), "{:3.2f}".format(weight))
            a = pyplot.gca().text(
                txt_x_pos, txt_y_pos, "{:3.2f}".format(weight), size=8, ha="center"
            )
            ...
            if self.previous_layer:
                for previous_layer_neuron in self.previous_layer.neurons:
                    self.__line_between_two_neurons(
                        neuron, previous_layer_neuron, weights[i, j], textoverlaphandler
                    )
                    i = i + 1
            j = j + 1
...
class TextOverlappingHandler:
    ...
    def __init__(self, width, height, grid_size=0.2):
        ...
        self.cells = np.ones(
            (int(np.ceil(width / grid_size)), int(np.ceil(height / grid_size))),
            dtype=bool,
        )
    ...
    def getspace(self, test_coordinates):
        ...
        if (
            self.cells[x_left_pos, y_botttom_pos]
            and self.cells[x_left_pos, y_top_pos]
            and self.cells[x_right_pos, y_top_pos]
            and self.cells[x_right_pos, y_botttom_pos]
        ):
...
class NeuralNetwork:
    ...
    def draw(self, directory, epoch, parent, idx, fitness, weights_list=None):
        ...
        overlaphandler = TextOverlappingHandler(
            ...
            * horizontal_distance_between_neurons,
            len(self.layers) * vertical_distance_between_layers,
            grid_size=0.2,
        )
        ...
        pyplot.axis("scaled")
        pyplot.axis("off")
        pyplot.title(f"Neural Network {idx} Epoch {epoch}", fontsize=15)
        ...
        figureName = f"{directory}/ep{epoch}_parent_{parent}_id{idx}.png"
        pyplot.savefig(figureName, dpi=300, bbox_inches="tight")
        pyplot.savefig(f"AIPlayer/runs/live_{parent}.png", dpi=300, bbox_inches="tight")
        pyplot.show()

    def save_draw(self, directory, epoch, parent, idx, fitness, weights_list=None):
        # vertical_distance_between_layers and horizontal_distance_between_neurons are the same with the variables of the same name in layer class
        vertical_distance_between_layers = 6
        horizontal_distance_between_neurons = 2
        overlaphandler = TextOverlappingHandler(
...
class DrawNN:
    ...
    def configure(self, neural_network, weights_list=None):
    ...
    def draw(self, epoch, parent, idx, fitness):
        widest_layer = max(self.neural_network)
        network = NeuralNetwork(widest_layer)
        for l in self.neural_network:
            network.add_layer(l)
        network.draw(self.directory, epoch, parent, idx, fitness, self.weights_list)

    def save_draw(self, epoch, parent, idx, fitness):
        widest_layer = max(self.neural_network)
        network = NeuralNetwork(widest_layer)
        for l in self.neural_network:
            network.add_layer(l)
        network.save_draw(
            self.directory, epoch, parent, idx, fitness, self.weights_list
        )
